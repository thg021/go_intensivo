...
func (suite *OrderRepositoryTestSuite) TestSavingOrder() {
	...
	err = suite.Db.QueryRow("select id, price, tax, price from orders where id = ?", order.ID).Scan(&orderResult.ID, &orderResult.Price, &orderResult.Tax, &orderResult.FinalPrice)
	suite.NoError(err)
